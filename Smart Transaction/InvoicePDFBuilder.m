...
-(void)drawStaticPart
{
    ...
    //NSLog(@"%@", _transaction);
    [self drawText:[[NSString alloc]initWithFormat:@"Invoice No : %@",[_transaction objectId]] inRect:CGRectMake(435, 55, invoicePageSize.width, invoicePageSize.height) withStyle:@"alignLeft" andSize:13];
    [self drawText:[[NSString alloc]initWithFormat:@"Date : %@",[_transaction createdAt]] inRect:CGRectMake(435, 85, invoicePageSize.width, invoicePageSize.height) withStyle:@"alignLeft" andSize:13];
//    [self drawText:[[NSString alloc]initWithFormat:@"Customer ID : %@",[PFUser currentUser].objectId] inRect:CGRectMake(435, 115, invoicePageSize.width, invoicePageSize.height) withStyle:@"alignLeft" andSize:13];
...
{
    int numberOfRecord = (int)[_transaction.transactionItems count] + 1;
...
-(void)addRecord
{
    ...
    for(TransactionItem* orderLine in [self.transaction transactionItems])
    {
        ...
        [self drawText:[product productName] inRect:CGRectMake(180, 155+initPosition, invoicePageSize.width, invoicePageSize.height) withStyle:@"alignLeft" andSize:13];
        [self drawText:[NSString stringWithFormat:@"%ld",(long)[orderLine quantity]] inRect:CGRectMake(397, 155+initPosition, invoicePageSize.width, invoicePageSize.height) withStyle:@"alignLeft" andSize:13];
        [self drawText:[NSString stringWithFormat:@"$%.1f",[product productPrice]] inRect:CGRectMake(492, 155+initPosition, invoicePageSize.width, invoicePageSize.height) withStyle:@"alignLeft" andSize:13];
        initPosition+=30;
    }
}
